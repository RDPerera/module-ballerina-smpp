 ...
 * material supplied per {@code Client}/{@code Listener} (NOT the JVM-global truststore
 * that jsmpp's stock {@code SSLSocketConnectionFactory} uses).
 *
 ...
 * nonetheless builds a fresh instance per bind/connect attempt, which also means
 * rotated trust material is picked up on the next rebind.
 *
...
public final class SmppSslConnectionFactory implements ConnectionFactory, RawConnectionFactory {
    ...
    /**
     * Builds the factory from resolved primitives (see types.bal's {@code ResolvedTls}).
    ...
    public RawConnection createRawConnection(String host, int port) throws IOException {
        ...
        // it is the force-close target for a bounded-close watchdog, because
        ...
        try {
            ...
            // from the connect-and-bind path ('start()/connect() or a rebind), not later
            // on the PDU reader thread. jsmpp overwrites SO_TIMEOUT with the enquire-link
            // timer right after this returns, so this value only bounds the handshake read.
