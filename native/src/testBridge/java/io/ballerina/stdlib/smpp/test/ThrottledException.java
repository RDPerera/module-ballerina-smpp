 ...
 * command_status {@code ESME_RTHROTTLED} - the connector's backpressure gate rejecting an
 * overflow PDU.
 *
 * <p>It exists as its own type because Ballerina's Java interop surfaces a thrown
 * exception's <em>class name</em> as {@code error.message()} (not its {@code getMessage()}
 * text), so a distinct class is how a {@code bal test} assertion can tell a throttle apart
 * from any other negative response. The command_status is kept available for completeness.
 */
