...
function testListenerConfigValidationRejectsOutOfBoundsValues() {
    record {|ListenerConfig config; string expect;|}[] cases = [
        ...
        {config: {maxConcurrentDispatch: 0}, expect: "maxConcurrentDispatch"},
        {config: {maxConcurrentDispatch: 5000}, expect: "maxConcurrentDispatch"},
        ...
        {config: {rebindPolicy: {initialRebindDelay: -1}}, expect: "initialRebindDelay"},
        {config: {rebindPolicy: {initialRebindDelay: 10, maxRebindDelay: 5}}, expect: "maxRebindDelay"},
        {config: {rebindPolicy: {backOffMultiplier: 0.5}}, expect: "backOffMultiplier"},
        {config: {rebindPolicy: {maxRebindAttempts: -2}}, expect: "maxRebindAttempts"}
    ];
    foreach var {config, expect} in cases {
        Listener|error result = new Listener("h", "s", "p", port = config.port,
...
function testListenerConfigValidationAcceptsBoundaryValues() {
    ...
    Listener|error ok = new ("h", "s", "p", port = 65535, maxConcurrentDispatch = 1024,
            gracefulStopTimeout = 0, enquireLinkInterval = 5, bindTimeout = 300, transactionTimeout = 300,
...
function testListenerBindTypeExcludesTransmitterAtCompileTime() {
    ...
    Listener|error asReceiver = new ("h", "s", "p", bindType = RECEIVER);
    test:assertTrue(asReceiver !is error, "RECEIVER must be a legal Listener bindType");
    Listener|error asTransceiver = new ("h", "s", "p", bindType = TRANSCEIVER);
