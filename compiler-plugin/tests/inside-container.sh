...
REPO=/home/ballerina/smpp
VERSION=$(grep '^version' "$REPO/ballerina/Ballerina.toml" | head -1 | sed 's/.*"\(.*\)"/\1/')

# Stale-plugin-jar guard: CompilerPlugin.toml pins the jar by literal version
# (restamped only by updateTomlFiles). On a dirty tree after a version bump,
# build/libs can still hold the OLD jar - bal pack would embed it silently and every
# fixture would then validate the PREVIOUS plugin. Fail loud instead.
if ! grep -q "smpp-compiler-plugin-${VERSION}.jar" "$REPO/ballerina/CompilerPlugin.toml"; then
    echo "CompilerPlugin.toml does not reference smpp-compiler-plugin-${VERSION}.jar - run updateTomlFiles"
    exit 72
fi
jarCount=$(ls "$REPO"/compiler-plugin/build/libs/smpp-compiler-plugin-*.jar 2>/dev/null | wc -l)
...
echo "=== packing ballerina/smpp:${VERSION} (with the compiler plugin) ==="
# Pack from a CONTAINER-LOCAL copy, never in place: on a Linux CI runner an earlier
# `bal test` container leaves ballerina/target root-owned on the host mount, and
# `bal pack` refuses ("target does not have write permissions"). The copy preserves
# the ../native and ../compiler-plugin relative jar paths the tomls reference, and
# leaves the workspace untouched as a bonus.
PKG=/tmp/pkgroot
mkdir -p "$PKG/ballerina" "$PKG/native/build/libs" "$PKG/compiler-plugin/build/libs"
(cd "$REPO/ballerina" && tar -c --exclude=./target .) | tar -x -C "$PKG/ballerina"
cp "$REPO"/native/build/libs/*.jar "$PKG/native/build/libs/"
cp "$REPO"/compiler-plugin/build/libs/smpp-compiler-plugin-*.jar "$PKG/compiler-plugin/build/libs/"
cd "$PKG/ballerina"
...
run_fixture() {
    local name="$1"
    local src="$REPO/compiler-plugin/tests/fixtures/$name"
...
count=0
for d in "$REPO"/compiler-plugin/tests/fixtures/*/; do
    run_fixture "$(basename "$d")"
    count=$((count + 1))
done

# A glob that matched nothing must not report success.
if [ "$count" -lt 10 ]; then
