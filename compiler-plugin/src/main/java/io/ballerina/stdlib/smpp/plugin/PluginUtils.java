...
/**
 * Type matching that mirrors the runtime's listener-attach type resolution exactly —
 * including the intersection handling.
 *
 * <p>Deliberately NOT a string-signature comparison: rendering the type to text and
 * comparing substrings rejects {@code readonly & smpp:Sms}, a shape the runtime accepts
 * at attach time. A plugin that re-broke it at compile time would reject a working
 * program. Matching is symbol-based: unwrap type-reference chains, and for
 * intersections scan the CONSTITUENT type symbols for the org/module/name match — the
 * same two-level resolution the runtime performs (a plain {@code getImpliedType} is not
 * sufficient on its own; the implied form does not carry the type name).
 */
public final class PluginUtils {
     ...
     * runtime's rejection of these shapes rather than letting a defaulted one be
     * silently skipped into nil.
     */
    ...
    private static boolean involvesCaller(TypeSymbol type, int depth) {
        ...
        if (unwrapped.typeKind() == TypeDescKind.INTERSECTION) {
            ...
            // union hunt above never sees it. Descend the constituents; isSmppType
            // alone cannot serve here because it deliberately never matches unions.
    ...
    /**
     * True if the type resolves (through references and intersections) to an error
     * type. A UNION does not pass: {@code error?} is rejected at attach and therefore
     * rejected here too.
     */
     ...
     * return types, which is exactly why this check is load-bearing: a
     * {@code returns string} handler runs, but its failures could never become a
    ...
    /** Locates the syntax node covering a line range (used by the code actions). */
